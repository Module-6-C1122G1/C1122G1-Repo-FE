...
export const findAll = async () => {
  try {
    const result = await axios.get("http://localhost:8080/api/public/discount");
    return result.data;
  } catch (error) {
    console.log(error);
  }
...
export const findByName = async (value, currentPage) => {
  const result = await axios.get(
    `http://localhost:8080/api/public/discount/list?name=${value}&page=${currentPage}`
  );
  return result;
};
...
export const createDiscount = async (discount) => {
  return await axios.post("http://localhost:8080/discount/create", {
    ...discount,
  });
};
...
export const updateDiscount = async (discount) => {
  return await axios.put(
    `http://localhost:8080/api/discount//update/${discount.id}`,
    { ...discount }
  );
};
