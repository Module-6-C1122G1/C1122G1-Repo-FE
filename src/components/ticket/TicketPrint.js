            ...
            <div className="container abc">
                <div>
                    <div className="border border-2 text-secondary">
                        ...
                        <div className="detail-rating p-3">
                            <div>
                                <h5 className="text-center">{props.ticketDetail.ticketSet[0].seat.showTime.film.nameFilm}</h5>
                            </div>
                            <table>
                                <thead>
                                {/*<tr style={{height: 39}}>*/}
                                ...
                                <tr style={{height: 39}}>
                                    ...
                                    <td>{props.ticketDetail.ticketSet[0].seat.showRoom.nameShowRoom}</td>
                                </tr>
                                <tr style={{height: 39}}>
                                    <th className="text-secondary">Ngày chiếu :</th>
                                    <td style={{fontFamily: "Roboto"}}>{format(new Date(props.ticketDetail?.ticketSet[0].seat.showTime.showDate), "dd/MM/yyyy")}</td>
                                </tr>
                                <tr style={{height: 39}}>
                                    <th className="text-secondary">Giờ chiếu:</th>
                                    <td>{props.ticketDetail.ticketSet[0].seat.showTime.showTime}</td>
                                </tr>
                                <tr style={{height: 39}}>
                                    <th className="text-secondary">Ghế:</th>
                                    <td>{props.ticketDetail.ticketSet.filter(element=>element.idTicket===props.idTicket)
                                        .map((element, index) => (element.seat.nameSeat))}
                                        <span>{props.ticketDetail.ticketSet.filter(element=>element.idTicket===props.idTicket)
                                            .map((element, index) => (element.seat.typeSeat.nameTypeSeat === 'VIP' ? ' (Ghế VIP)' : ' (Ghế thường)'))}</span>
                                    </td>
                                </tr>
                                <tr className="" style={{height: 39}}>
                                    <th className="text-secondary">Giá:</th>
                                    <td>{props.ticketDetail.ticketSet.filter(element=>element.idTicket===props.idTicket)
                                        .map((element, index) => (element.priceAfterDiscount)
