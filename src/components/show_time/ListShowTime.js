...
export function ListShowTime() {
    ...
    useEffect(() => {
        const listShowTime = async () => {
            const result = await ShowTimeService.findAllShowTime(searchAndPage);
            console.log(result)
            try {
                setShowTimeList(result.content);
    ...
    const handleDelete = async () => {
        await ShowTimeService.deleteShowTime(showTimes.idFilm);
        const result = await ShowTimeService.findAllShowTime(searchAndPage);
        setShowTimes(result)
    ...
    const handleSearch = async (e) => {
        e.preventDefault()
        return await axios.get(`http://localhost:8080/api/public/showTime?q=${value}`)
            .then((res) => {
                ...
                <div className="container mx-auto my-5 col-8">
                    <div style={{boxShadow: "1px 3px 10px 5px rgba(0, 0, 0, 0.2)"}}>
                        ...
                        <div className="row ">
                            ...
                            <div>
                                <Link to={`/admin/showtime/create`} style={{display: "inline-block", float: "right", marginRight: "2%"}}>
                                                ...
                                                >
                                                    ...
                                                    {showTimeList.map((showTime, index) => (
                                                        <tr key={index} className="text-center">
                                                            <td>{index + 1}</td>
                                                            <td>{showTime.film.nameFilm}</td>
                                                            <td>{showTime.showDate}</td>
                                                            <td>{showTime.showTime}</td>
                                                            <td>{showTime.showRoom.nameShowRoom}</td>
                                                            <td className="d-flex justify-content-center">
                                                                <Link to={`/admin/showtime/edit/${showTime.idShowTime}`}>
                                                                ...
                                                                <button type="submit" className="btn btn-danger" data-bs-toggle="modal"
                                                                        ...
                                                                        onClick={() => getData(showTime.idShowTime)}
                                                                >
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <span>Bạn có muốn xóa:</span> <span style={{color: 'red'}} className="font-monospace">{showTimes?.film.nameFilm} ?</span>
                        </div>
